...
float shadowIntensity = 0.8f;     // How dark the shadows are (0.0 to 1.0)
float shadowOrthoBoxSize = 10.0f; // The size of the shadow orthographic projection box
float shadowDistance = 10.0f;     // The distance from the center where the shadow map is rendered from
...
void render(const Shader& shader_light, const Shader& shader_phong, const Shader& shader_depth, const Geometry& quad, const Geometry& cube, const Geometry& pyramid, const Geometry& sphere, const Texture& texture_material, const Texture& texture_reflection, const Texture& texture_normal, uint32_t fbo, uint32_t text_fbo) {
  ...
  glm::vec3 lightPosForShadow = -lightDirWorld * shadowDistance; // Fake position for dir light
  glm::mat4 lightProjection = glm::ortho(-shadowOrthoBoxSize, shadowOrthoBoxSize, -shadowOrthoBoxSize, shadowOrthoBoxSize, k_shadow_near, k_shadow_far);
  ...
  // ---------------------------------------
  {
    ...
    shader_phong.set("shadowIntensity", shadowIntensity);
    ...
    shader_phong.set("normalMat", glm::transpose(glm::inverse(glm::mat3(view * model_pyr2_phong))));
    pyramid.render();
    ...
    shader_phong.set("normalMat", glm::transpose(glm::inverse(glm::mat3(view * model_pyr3_phong))));
    pyramid.render();
    ...
    shader_phong.set("normalMat", glm::transpose(glm::inverse(glm::mat3(view * model_pyr5_phong))));
    pyramid.render();
    ...
    shader_phong.set("normalMat", glm::transpose(glm::inverse(glm::mat3(view * model_pyr6_phong))));
    pyramid.render();
...
int main(int argc, char* argv[]) {
  ...
  while (window->isAlive()) {
    ...
    //update();
    render(shader_light, shader_phong, shader_depth, quad, cube, pyramid, sphere, texture_material, texture_reflection, texture_normal, fbo.first, fbo.second);
