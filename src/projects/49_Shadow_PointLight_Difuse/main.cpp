...
void render(const Shader& shader_light, const Shader& shader_phong, const Quad& quad, const Cube& cube, const Pyramid& pyramid, const Sphere& sphere, const Texture& t_albedo, const Texture& t_specular, const Shader& shader_depth, float time) {
  ...
  glm::vec3 lightDirWorldDifuse = glm::normalize(-currentLightPosDifuse);
  Shadow* shadow = Shadow::instance();
  glm::mat4 lightSpaceMatrixDifuse = shadow->getLightSpaceMatrix(lightDirWorldDifuse);
  // ========================================================================
  // PASS 1: DEPTH MAP
  // ========================================================================
shadow->bindFBO();
  ...
  shader_depth.use();
  shader_depth.set("lightSpaceMatrixDifuse", lightSpaceMatrixDifuse);
...
int fbWidth, fbHeight;
  ...
  shader_phong.set("lightSpaceMatrixDifuse", lightSpaceMatrixDifuse);
  glActiveTexture(GL_TEXTURE2);
  glBindTexture(GL_TEXTURE_2D, shadow->getDepthMap());
  shader_phong.set("shadowMapDifuse", 2);
  shader_phong.set("shadowIntensityDifuse", shadow->getIntensity());
