...
void main() {
	...
	if (useTexture) {
		albedo_map = vec3(texture(texture_diffuse1, uv)) * colorTint;
		specular_map = vec3(texture(texture_specular1, uv));
		normal_map = vec3(texture(texture_normal1, uv)); // NORMAL MAPPING
	} else {
		albedo_map = colorTint;
		specular_map = vec3(0.5);
		normal_map = vec3(0.5, 0.5, 1.0); // Flat normal
	}
	...
	if (useTexture) {
		...
		norm = normalize(TBN * normalize(mapped_normal)); // NORMAL MAPPING
	} else {
		norm = normalize(TBN[2]); // Raw geometric normal, bypass TBN multiplication which can yield NaN
	}
