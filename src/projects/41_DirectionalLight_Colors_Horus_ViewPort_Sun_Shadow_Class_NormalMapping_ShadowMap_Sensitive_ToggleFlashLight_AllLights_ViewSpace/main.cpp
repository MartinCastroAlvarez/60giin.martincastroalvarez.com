...
void drawSphere(const Shader& shader, const Sphere& sphere, const glm::vec3& pos, const glm::mat4& view = glm::mat4(1.0f)) {
  glm::mat4 m = glm::mat4(1.0f);
  m = glm::translate(m, pos);
...
void render(const Shader& shader_light, const Shader& shader_phong, const Shader& shader_depth, const Shader& shader_fbo, const Geometry& quad, const Geometry& cube, const Geometry& pyramid, const Geometry& sphere, const Model& horus, const Texture& texture_material, const Texture& texture_reflection, const Texture& texture_normal, float time) {
  ...
  shader_depth.set("model", horusModelMat);
  horus.render(shader_depth);

  // NEW SHAPES DEPTH
  glm::vec3 posCube(sin(time) * 3.0f, -3.0f, 0.0f);
  glm::vec3 posSphere(sin(time + 2.0f) * 3.0f, -3.0f, 2.0f);
  glm::vec3 posPyramid(sin(time + 4.0f) * 3.0f, -3.0f, -2.0f);

  drawCube(shader_depth, static_cast<const Cube&>(cube), posCube);
  drawSphere(shader_depth, static_cast<const Sphere&>(sphere), posSphere);
  drawPyramid(shader_depth, static_cast<const Pyramid&>(pyramid), posPyramid);
  ...
  // ---------------------------------------
  {
    ...
    texture_normal.use(shader_phong, "texture_normal1", 3); // NORMAL MAPPING
    shader_phong.set("normalIntensity", normalIntensity);   // NORMAL MAPPING
    shader_phong.set("shininess", 64);
    shader_phong.set("colorTint", glm::vec3(1.0f)); // DEFAULT NO TINT
    shader_phong.set("useTexture", true); // DEFAULT USE TEXTURE
    ...
    shader_phong.set("normalMat", glm::transpose(glm::inverse(glm::mat3(sceneView * horusModelMat))));
    horus.render(shader_phong);

    // DRAW COLORED OSCILLATING SHAPES
    shader_phong.set("useTexture", false);

    shader_phong.set("colorTint", glm::vec3(1.0f, 0.0f, 0.0f));
    drawCube(shader_phong, static_cast<const Cube&>(cube), posCube, sceneView);

    shader_phong.set("colorTint", glm::vec3(0.0f, 1.0f, 0.0f));
    drawSphere(shader_phong, static_cast<const Sphere&>(sphere), posSphere, sceneView);

    shader_phong.set("colorTint", glm::vec3(0.0f, 0.0f, 1.0f));
    drawPyramid(shader_phong, static_cast<const Pyramid&>(pyramid), posPyramid, sceneView);

    // RESET TINT AND TEXTURE
    shader_phong.set("colorTint", glm::vec3(1.0f));
    shader_phong.set("useTexture", true);
  }
  };
...
int main(int argc, char* argv[]) {
  ...
  const Pyramid pyramid = Pyramid(1.0f);
