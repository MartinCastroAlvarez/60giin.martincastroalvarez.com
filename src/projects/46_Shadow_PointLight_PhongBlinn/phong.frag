...
float ShadowCalculation(vec4 fragPosLightSpace, vec3 lightDir, vec3 normal)
{
    ...
    for(int x = -1; x <= 1; ++x)
    {
        for(int y = -1; y <= 1; ++y)
        {
            float pcfDepth = texture(shadowMapPhongBlinn, projCoords.xy + vec2(x, y)).r; 
...
void main() {
	...
	vec3 phong = (ambient + (1.0 - shadow * shadowIntensityPhongBlinn) * (diffuse + specular)) * attenuation;
