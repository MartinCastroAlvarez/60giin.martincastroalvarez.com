...
// SHADOW CONSTANTS
const float SHADOW_GOURAUD_ORTHO_BOX_SIZE = 10.0f;
const float SHADOW_GOURAUD_DISTANCE = 10.0f;
const float SHADOW_GOURAUD_INTENSITY = 0.8f;
...
int main(int argc, char* argv[]) {
  ...
  Shadow* shadow = Shadow::instance();
  shadow->init();
  shadow->setOrthoBoxSize(SHADOW_GOURAUD_ORTHO_BOX_SIZE);
  shadow->setDistance(SHADOW_GOURAUD_DISTANCE);
  shadow->setIntensity(SHADOW_GOURAUD_INTENSITY);
